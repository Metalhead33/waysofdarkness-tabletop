...
\item For the sake of convenience and simplicity, at some places, I will still refer to the classical 9-alignment system.
\end{itemize}
...
\section{Classes}
Unlike in other roleplaying games, classes do not impose too heavy restrictions on player characters, with the exception of Magic. Some classes may forbid certain feats, and some classes may have unique feats, but no classes impose restrictions that would realistically make no sense from a real-life perspective \textit{(such as forbidding Clerics from using bladed weapons, forbidding Magicians from using any weapons other than clubs and daggers, etc.)} - such restrictions are meant to be softly self-imposed and adhered to for the sake of roleplaying \textit{(e.g. a Cleric not using bladed weapons because his or her religion forbids it)}, rather than something integrated into game mechanics.\newline
Characters can also be classless entirely, which comes with the advantage of no attribute requirements and restrictions on taking on feats, but also no bonus starting feats, and no unique feats at all.\newline
A character may hold two classes, but upon taking up their second class, they will stop advancing in the first one, only being able to level up in the second one. Dualclassing is the only method for characters who don't initially possess magic to learn magic.\newline
Each module brings its own roster of classes, but all of them can be neatly grouped into 5 class groups:
\begin{itemize}
\item \textbf{Fighter Classes:} Fighter type classes focus on raw strength, maximizing their own hitpoints and damage inflicted on the enemy. Not much else is there to say about them. These classes typically start with bonus feats in weapon expertise and armour-wearing, and may have one or two unique feats.
\item \textbf{Rogue Classes:} Rogue type classes focus on stealth and covert attacks, favouring speed and agility over brute strength. These classes tend to get bonuses when it comes to feats like forgery, smuggling, poisoning, stealth and backstabbing, may even get unique feats that aid in these tasks.
\item \textbf{Magician Classes:} Magician type classes enabled the character to use Arcane Magic, which will be described in a later section. The most important thing to keep in mind, is that Magician type characters learn their spells from books and scrolls instead of spontaneously gaining them, and can learn a wide variety of spells. Nost Magician type classes are very poor at melee and ranged combat alike, with some notable exceptions, like Battlemages and Spellthieves. \textcolor{red}{Characters with a Magician type class are explicitly forbidden from dualclassing with a Cleric type class or a Druid type class.}
\item \textbf{Cleric Classes:} Cleric type classes enabled the character to use Clerical Magic, which will be described in a later section. The most important thing to keep in mind, is that Cleric type characters often spontaneously gain new spells in addition to being able to learn them from scrolls, but their repertoire of learnable spells is usually limited, e.g. Good-aligned Clerics are limited to healing spells, anti-undead spells and maybe a few destructive ones. To compensate for the limited repertoire of spells and limited access to destructive magicks, Cleric type classes tend to be better at melee combat than Magician type classes. \textcolor{red}{Characters with a Cleric type class are explicitly forbidden from dualclassing with a Magician type class or a Druid type class.}
\item \textbf{Druid Classes:} Druid type classes enabled the character to use Nature Magic, which will be described in a later section. In some ways, Druidic Nature Magic combines the best of Arcane and Clerical magic, offering the diversity of the earlier while also giving some divine fallback like the latter. Of course, everything has a price, and Druid type characters face a lot of restrictions when it comes to feats. \textcolor{red}{Characters with a Druid type class are explicitly forbidden from dualclassing with a Magician type class or a Cleric type class.}
\end{itemize}
